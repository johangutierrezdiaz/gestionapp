		...
		for (var j = 0; j < apertura_cierre.registros; j++) {
			if (operacion.datos[k].id_operacion === apertura_cierre.datos[j].id_operacion) {
				...
				if (id_oficina != "") {
					...
					fila.push(OFICINAS_BIZAGI.includes(id_oficina) ? OFICINAS_DATASTUDIO[OFICINAS_BIZAGI.indexOf(id_oficina)] : oficina);
		...
		if (sw) {
			for (let m = 0; m < apertura_cierre.registros; m++) {
				if (apertura_cierre.datos[m].id_operacion == novedades_personal.datos[k].id_operacion) {
					...
					fila.push(OFICINAS_BIZAGI.includes(id_oficina) ? OFICINAS_DATASTUDIO[OFICINAS_BIZAGI.indexOf(id_oficina)] : nombre_oficina);
